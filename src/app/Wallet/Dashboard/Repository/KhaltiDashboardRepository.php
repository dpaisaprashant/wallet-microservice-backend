...
class KhaltiDashboardRepository extends DashboardRepository implements DashboardStatsRepositoryInterface
{
    ...
    {
        return KhaltiUserTransaction::whereDate('created_at', '>=', $this->from)
            ->whereDate('created_at', '<=', $this->to);
    }

    public function pendingDisputeBuilder()
    {
        return Dispute::whereTransactionType(KhaltiUserTransaction::class)
    ...
    public function resolvedDisputeBuilder()
    {
        ...
        return Dispute::whereIn('id', $handlerIds)->whereTransactionType(KhaltiUserTransaction::class)
            ->where('user_dispute_status', Dispute::USER_DISPUTE_STATUS_CLEARED);
    }
    ...
    {
        return TransactionEvent::whereTransactionType(KhaltiUserTransaction::class)
            ->get()
            ->groupBy('vendor')
    ...
    {
        return Clearance::with('clearanceTransactions')->whereClearanceType(Clearance::TYPE_KHALTI)->latest()->take(4)->get();
    }
