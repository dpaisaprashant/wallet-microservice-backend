...
class PayPointDashboardRepository extends DashboardRepository implements DashboardStatsRepositoryInterface
{
    ...
    {
        return UserTransaction::whereDate('created_at', '>=', $this->from)
            ->whereDate('created_at', '<=', $this->to);
    }

    public function pendingDisputeBuilder()
    {
        return Dispute::whereTransactionType(UserTransaction::class)
    ...
    public function resolvedDisputeBuilder()
    {
        ...
        return Dispute::whereIn('id', $handlerIds)->whereTransactionType(UserTransaction::class)
            ->where('user_dispute_status', Dispute::USER_DISPUTE_STATUS_CLEARED);
    }
    ...
    {
        return TransactionEvent::whereTransactionType(UserTransaction::class)
            ->get()
            ->groupBy('vendor')
    ...
    {
        return Clearance::with('clearanceTransactions')->whereClearanceType(Clearance::TYPE_PAYPOINT)->latest()->take(4)->get();
    }
