...
@section('content')
    ...
    <div class="wrapper wrapper-content animated fadeInRight">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <select data-placeholder="Select User Profile View ..." class="chosen-select" id="user_profile_view"
                                        tabindex="2" name="user_profile_view">
                                    <option value="" disabled>Select User Profile View...</option>
                                    <option value="kyc">KYC</option>
                                    <option value="agent">Agent</option>
                                    <option value="audit_trails">All Audit Trails</option>
                                    <option value="login_history">User Login History Audit</option>
                                    <option value="referral_code">Referral Code</option>
                                    <option value="referral_bonus">Referral Bonus</option>
                                    <option value="limits">Limits</option>
                                    <option value="wallet">Wallet</option>
                                    <option value="vendor_graph">Vendor Graph</option>
                                    <option value="transaction_graph">Transaction Graph</option>
        ...
        <div class="kyc" id="user-profile-kyc">
{{--            @include('admin.user.profile.tabs.kyc')--}}
        </div>

        <div class="wallet" id="user-profile-wallet">
...
{{--    </script>--}}
    ...
    <script>
        const user_profile_btn = document.querySelector('#user_profile_view_btn');
        const user_profile_view = document.querySelector('#user_profile_view');
        user_profile_btn.onclick = function (event){
            $("#overlay").fadeIn(300);
            document.querySelectorAll('[id^="user-profile"]').forEach(el =>{
                el.style.display = "none";
            });
            if(user_profile_view.value === 'kyc'){
                console.log('hey kyc');
                // kycView();
            }
            document.querySelector('#user-profile-'.concat(user_profile_view.value)).style.display = "block";
            $("#overlay").fadeOut(300);

            // document.getElementById('kyc_div').innerHTML = response;
        }


        {{--function kycView( $ ){--}}
        {{--        $.ajax({--}}
        {{--            url:{{route('user.new_profile.kyc',$user->id)}},--}}
        {{--            method:'GET',--}}
        {{--            success: function (resp){--}}
        {{--                $("#user-profile-kyc").html(resp);--}}
        {{--            },--}}
        {{--            error: function (resp){--}}
        {{--                console.log(resp);--}}
        {{--                alert('error');--}}
