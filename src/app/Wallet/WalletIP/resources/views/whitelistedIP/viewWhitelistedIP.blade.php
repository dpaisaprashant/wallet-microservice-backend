                    ...
                    <div class="ibox-content">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                ...
                                <tbody>
                                @foreach($whitelistedIPs as $key=>$whitelistedIP)
                                    <tr>
                                        <td> {{ $key+1 }}</td>
                                        <td> {{ $whitelistedIP->ip }}</td>
                                        <td> {{ $whitelistedIP->title }}</td>
                                        ...
                                        <td class="center">
                                            ...
                                            <form action="{{ route('whitelistedIP.delete',$whitelistedIP->id) }}" method="POST">
                                                ...
                                                <button                                                    
                                                    ...
                                                    rel="{{ $whitelistedIP->id }}"><i
                                                        class="fa fa-trash"></i>
                                                </button>              

                                                <button id="resetBtn-{{ $whitelistedIP->id }}"
                                                    style="display: none" type="submit"
                                                    href="{{ route('whitelistedIP.delete',$whitelistedIP->id) }}"
                            ...
                            {{ $whitelistedIPs->appends(request()->query())->links() }}
                        </div>
...
@section('scripts')
    ...
    <script>
        $('.reset').on('click', function (e) {
            e.preventDefault();
            let whitelistedIP_Id = $(this).attr('rel');
            swal({
                title: "Are you sure?",
                text: "Whitelisted IP will be deleted",
                type: "warning",
                ...
                $('#resetBtn-' + whitelistedIP_Id).trigger('click');
                swal.close();
