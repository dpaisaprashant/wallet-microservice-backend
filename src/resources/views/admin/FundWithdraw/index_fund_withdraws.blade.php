...
{{--                                    </div>--}}
                                    ...
                                    <div>
                                        <button class="btn btn-sm btn-primary float-right m-t-n-xs" type="submit" formaction="{{ route('fund-withdraw,index') }}"><strong>Filter</strong></button>
                                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover dataTables-example" title="Complete transactions list">
                                ...
                                <tr>
                                    ...
                                    <th>Withdrawn Pre Transaction ID</th>
                                    <th>Transaction Event ID</th>
                                    <th>Before Balance</th>
                                    <th>After Balance</th>
                                    <th>Before Bonus Balance</th>
                                    <th>After Bonus Balance</th>
                                    <th>Description</th>
                                    <th>Date</th>
{{--                                    <th>Action</th>--}}
                                </tr>
                                ...
                                <tbody>
                                @foreach($withdrawn_funds as $withdrawn_fund)
                                    <tr class="gradeC">
                                        <td>{{ $loop->index + ($withdrawn_funds->perPage() * ($withdrawn_funds->currentPage() - 1)) + 1 }}</td>
                                        <td>{{ $withdrawn_fund->pre_transaction_id }}</td>
                                        <td>{{ $withdrawn_fund->withdraw_pre_transaction_id }}</td>
                                        <td>{{ $withdrawn_fund->transaction_event_id ?? null }}</td>
                                        <td>{{ $withdrawn_fund->before_balance}}</td>
                                        <td>{{ $withdrawn_fund->after_balance }}</td>
                                        <td>{{ $withdrawn_fund->before_bonus_balance }}</td>
                                        <td>{{ $withdrawn_fund->after_bonus_balance}}</td>
                                        <td>{{ $withdrawn_fund->description }}</td>
                                        <td>{{ $withdrawn_fund->created_at }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                            {{ $withdrawn_funds->appends(request()->query())->links() }}
                        </div>
                    </div>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function (e) {
            let a = "Showing {{ $withdrawn_funds->firstItem() }} to {{ $withdrawn_funds->lastItem() }} of {{ $withdrawn_funds->total() }} entries";
